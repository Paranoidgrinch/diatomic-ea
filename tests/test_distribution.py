...
def test_release_version_is_consistent() -> None:
    ...
    assert re.search(r'^version\s*=\s*"1\.0\.0rc1"\s*$', pyproject, re.MULTILINE)
...
def test_release_metadata_and_gui_entry_point() -> None:
    ...
    assert 'diatomic-ea-gui = "diatomic_ea.desktop_gui:main"' in pyproject
...
def test_windows_installers_are_repo_location_independent() -> None:
    ...
    uninstaller = (ROOT / "scripts/uninstall_windows.ps1").read_text(encoding="utf-8")
...
def test_linux_installer_is_native_compute_install() -> None:
    installer = (ROOT / "scripts/install_linux.sh").read_text(encoding="utf-8")
...
def test_readme_is_release_candidate_not_early_development() -> None:
