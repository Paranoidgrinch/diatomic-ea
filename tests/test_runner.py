"""Tests for resumable fast-grid orchestration."""
...
def test_run_persists_completed_tasks(
    tmp_path,
) -> None:
    first = make_task(1.5)
    ...
    with patch(
        ...
        summary = execute_fast_grid_resumable(
            plan,
            store=store,
            max_workers=2,
        )
...
def test_resume_skips_finished_task(
    tmp_path,
) -> None:
    first = make_task(1.5)
    second = make_task(1.525)
...
def test_completed_plan_runs_no_workers(
    tmp_path,
) -> None:
    task = make_task(1.5)
    plan = make_plan((task,))
